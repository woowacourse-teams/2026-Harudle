...
- S3 어댑터가 호출 전에 감지한 Object Key와 객체 크기 검증 실패
- 예상하지 못한 OAuth 공급자 인증 실패
- CSRF 실패처럼 보안상 관찰할 가치가 있는 사건
- 이미지 정리와 같은 보상 작업 실패

외부 연동 실패에는 원본 예외 타입과 민감정보를 제거한 스택 프레임을 포함한다. 외부 SDK 예외의 원문 메시지와 원인 체인은 기록하지
않는다. 사용자 행동으로 예상 가능한 인증 실패에는 스택 트레이스를 포함하지 않는다.
...
- `PROVIDER_AUTHENTICATION_FAILED`, `INVALID_PROVIDER_PROFILE`은 `WARN`으로 기록한다.
...
- 요청 구성, 응답 매핑 및 내부 불변식 오류는 `ERROR`로 기록한다.
...
- 빈 Object Key나 허용 범위를 벗어난 객체 크기처럼 S3 호출 전에 감지한 데이터 검증 실패는
  `REQUEST_VALIDATION_ERROR`로 분류하고 `WARN`으로 기록한다.
- 요청 객체 구성, S3 응답 처리와 내부 불변식 오류는 `ERROR`로 기록한다.
