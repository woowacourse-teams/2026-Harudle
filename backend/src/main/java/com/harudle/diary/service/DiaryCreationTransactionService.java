...
public class DiaryCreationTransactionService {
    ...
    public DiaryCreationTransactionService(
            ...
            Clock clock,
            @Value("${harudle.generation.processing-timeout:PT15M}") Duration processingTimeout
    ) {
    ...
    private void interruptIfStale(ComicGeneration generation) {
        ...
        if (generation.getUpdatedAt().isBefore(clock.instant().minus(processingTimeout))) {
            generation.interrupt(clock.instant());
        }
    }
